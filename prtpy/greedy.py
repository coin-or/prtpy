...
def greedy(
    ...
    objective: Objective = None,  # Not used
    outputtype: OutputType = Partition,
):
    """
    Partition the given items using the greedy number partitioning algorithm.

    >>> greedy(numbins=2, items=[1,2,3,3,5,9,9], outputtype=EntirePartition)
    [[9, 5, 2], [9, 3, 3, 1]]
    >>> greedy(numbins=3, items=[1,2,3,3,5,9,9], outputtype=EntirePartition)
    [[9, 2], [9, 1], [5, 3, 3]]
    >>> list(greedy(numbins=3, items=[1,2,3,3,5,9,9], outputtype=Sums))
    [11.0, 10.0, 11.0]
    >>> greedy(numbins=3, items=[1,2,3,3,5,9,9], outputtype=LargestSum)
    11.0

    >>> from partition import partition
    >>> partition(algorithm=greedy, numbins=3, items={"a":1, "b":2, "c":3, "d":3, "e":5, "f":9, "g":9})
    [['f', 'b'], ['g', 'a'], ['e', 'c', 'd']]
    >>> partition(algorithm=greedy, numbins=2, items={"a":1, "b":2, "c":3, "d":3, "e":5, "f":9, "g":9}, outputtype=Sums)
    array([16., 16.])
    """
