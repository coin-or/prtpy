...
class TestImprovedBinCompletion(unittest.TestCase):
    ...
    def test_fits_one_bin(self):
        assert algorithm(BinsKeepingContents(), binsize=5, items=[
                         1, 1, 1, 1, 1]).bins == [[1, 1, 1, 1, 1]]
        assert algorithm(BinsKeepingContents(), binsize=10,
                         items=[10]).bins == [[10]]
        assert algorithm(BinsKeepingContents(), binsize=15, items=[
                         1, 2, 3, 4, 5]).bins == [[5, 4, 3, 2, 1]]
        assert algorithm(BinsKeepingContents(), binsize=10,
                         items=[1, 2, 3]).bins == [[3, 2, 1]]
        assert algorithm(BinsKeepingContents(), binsize=3,
                         items=[1, 2, 0, 0]).bins == [[2, 1]]
    ...
    def test_zero_bins(self):
        ...
        assert algorithm(BinsKeepingContents(),
                         binsize=1, items=[0]).bins == []
        assert algorithm(BinsKeepingContents(), binsize=1,
                         items=[0, 0, 0, 0]).bins == []
        assert algorithm(BinsKeepingContents(), binsize=0, items=[]).bins == []
        assert algorithm(BinsKeepingContents(),
                         binsize=0, items=[0]).bins == []
        assert algorithm(BinsKeepingContents(), binsize=0,
                         items=[0, 0, 0, 0]).bins == []
    ...
    def test_variant_examples(self):
        assert algorithm(BinsKeepingContents(), binsize=100, items=[
                         99, 99, 99, 99, 99]).bins == [[99], [99], [99], [99], [99]]
        assert algorithm(BinsKeepingContents(), binsize=100, items=[
                         3, 9, 12, 15]).bins == [[15, 12, 9, 3]]
        assert algorithm(BinsKeepingContents(), binsize=100, items=[
                         99, 90, 60, 30, 10]).bins == [[99], [90, 10], [60, 30]]
        ...
        assert algorithm(BinsKeepingContents(), binsize=100, items=[
                         5, 11, 14, 39, 42, 81, 0, 0]).bins == [[81, 14, 5], [42, 39, 11]]
