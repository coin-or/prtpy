...
def schroeppel_shamir(s):
    ...
    min_heap = MinHeap()  # create a new min heap
    for i in list_sorted1:
        min_heap.push(i)

    # deep copy
    deep_copied_heap = copy.deepcopy(min_heap)

    # empty the original heap
    while len(min_heap.heap) is not 0:
        min_heap.pop()

    list_sorted2 = [[list_of_sum_b0[i], list_of_sum_b1[j]] for i in range(len(list_of_sum_b0)) for j in
                    range(len(list_of_sum_b1))]
    max_heap = MaxHeap()  # create a new max heap
    for i in list_sorted2:
        max_heap.push(i)

    # deep copy
    deep_copied_heap2 = copy.deepcopy(max_heap)

    # empty the original heap
    while len(max_heap.heap) is not 0:
        max_heap.pop()
    return deep_copied_heap.heap, deep_copied_heap2.heap
